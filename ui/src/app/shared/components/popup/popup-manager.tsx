    ...
    public confirm(title: string, message: string | React.ComponentType): Promise<boolean> {
        ...
        return new Promise(resolve => {
            ...
            this.popupPropsSubject.next({
                ...
                footer: (
                    <div>
                        <button className='argo-button argo-button--base' onClick={() => closeAndResolve(true)}>
                            OK
                        </button>{' '}
                        <button className='argo-button argo-button--base-o' onClick={() => closeAndResolve(false)}>
                            Cancel
                        </button>
                    </div>
                )
    ...
    public prompt(
        ...
        return new Promise(resolve => {
            ...
            this.popupPropsSubject.next({
                ...
                content: () => (
                    <Form validateError={settings && settings.validate} onSubmit={onSubmit} getApi={api => (formApi = api)}>
                        {api => <form onSubmit={api.submitForm}>{form(api)}</form>}
                    </Form>
                ),
                footer: (
                    <div>
                        <button className='argo-button argo-button--base' onClick={() => formApi.submitForm(null)}>
                            OK
                        </button>{' '}
                        <button className='argo-button argo-button--base-o' onClick={() => closeAndResolve(null)}>
                            Cancel
                        </button>
                    </div>
                )

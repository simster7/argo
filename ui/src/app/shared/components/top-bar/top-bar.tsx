        ...
        anchor={() => (
            <div
                className={classNames('top-bar__filter', {
                    'top-bar__filter--selected': filter.selectedValues.length > 0
                })}>
    ...
    <div className='top-bar__breadcrumbs'>
        {(breadcrumbs || []).map((breadcrumb, i) => {
            ...
            if (i === breadcrumbs.length - 1) {
                nodes.push(
                    <span key={breadcrumb.title} className='top-bar__breadcrumbs-last-item'>
                        {breadcrumb.title}
                    </span>
                );
            } else {
...
const renderToolbar = (toolbar: Toolbar) => (
    <div className='top-bar row' key='tool-bar'>
        <div className='columns small-9 top-bar__left-side'>{toolbar.actionMenu && renderActionMenu(toolbar.actionMenu)}</div>

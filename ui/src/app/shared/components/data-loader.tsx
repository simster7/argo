...
export class DataLoader<D = {}, I = {}> extends React.Component<
    LoaderProps<I, D>,
    {
        loading: boolean;
        dataWrapper: {data: D};
...
> {
    ...
    public render() {
        ...
        if (this.state.error) {
            const error = (
                <p style={style}>
                    Failed to load data, please <a onClick={() => this.reload()}>try again</a>.
                </p>
            );
    ...
    private async loadData() {
        if (!this.state.error && !this.state.loading && (this.state.dataWrapper == null || this.state.inputChanged)) {
            this.setState({
                ...
                loading: true,
                inputChanged: false,
                dataWrapper: this.props.noLoaderOnInputChange ? this.state.dataWrapper : null
            });
            try {
                ...
                if ((res as Promise<D>).then) {
                    ...
                    this.ensureUnsubscribed();
                    this.subscription = (res as Observable<D>).subscribe(
                        (data: D) => this.setState({loading: false, dataWrapper: {data}}),
